...
export function recordStudyActivity() {
  ...
  try {
    activity = JSON.parse(getSlotItem(ACT) || '{"dates":[]}');
  } catch {}
  setSlotItem(
    ACT,
    JSON.stringify({
      dates: Array.from(new Set([...(activity.dates || []), day()])),
...
export function getStudyStreak() {
  let dates: string[] = [];
  try {
    dates = JSON.parse(getSlotItem(ACT) || '{"dates":[]}').dates || [];
  } catch {
    return 0;
  }
...
export function restoreLocalData(payload: BackupPayload) {
  if (payload?.app !== "Chinese Mission Việt" || !payload.data) {
...
export function downloadBackup() {
  const backup = new Blob([JSON.stringify(collectLocalData(), null, 2)], {
